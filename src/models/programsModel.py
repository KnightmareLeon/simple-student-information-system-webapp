    ...
    def _create_table(cls):
        ...
        try:
            ...
            query = (
                ...
                "        ON UPDATE CASCADE\n"
                "        ON DELETE SET NULL,\n"
                "    CONSTRAINT program_code_format CHECK (\"Code\"::text ~ '^[[:alpha:]]+( [[:alpha:]]+)*$'::text AND length(\"Code\"::text) <= 20) NOT VALID,\n"
                "    CONSTRAINT program_name_format CHECK (\"Name\"::text ~ '^[[:alpha:]]+( [[:alpha:]]+)*$'::text AND length(\"Name\"::text) <= 100) NOT VALID\n"
                ")\n"
