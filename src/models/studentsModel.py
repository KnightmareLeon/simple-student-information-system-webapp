    ...
    def _create_table(cls):
        ...
        try:
            ...
            query = (
                ...
                "        ON UPDATE CASCADE\n"
                "        ON DELETE SET NULL,\n"
                "    CONSTRAINT first_name_format CHECK (\"FirstName\"::text ~ '^[[:alpha:]]+( [[:alpha:]]+)*$'::text AND length(\"FirstName\"::text) <= 100) NOT VALID,\n"
                "    CONSTRAINT last_name_format CHECK (\"LastName\"::text ~ '^[[:alpha:]]+( [[:alpha:]]+)*$'::text AND length(\"LastName\"::text) <= 100) NOT VALID,\n"
                "    CONSTRAINT id_format CHECK (\"ID\" ~ '^(?:19(?:6[7-9]|[7-9][0-9])|20[0-9]{2}|2100)-(?:000[1-9]|00[1-9][0-9]|0[1-9][0-9]{2}|[1-9][0-9]{3})$'::text) NOT VALID\n"
                ")\n"
